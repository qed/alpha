...
function Icon({ name, size = 16 }: { name: string; size?: number }) {
  const icons: Record<string, React.ReactNode> = {
    quote: (
      <path d="M3 8c0-2.5 2-4 4-4v2c-1 0-2 .5-2 2h2v4H3V8zm8 0c0-2.5 2-4 4-4v2c-1 0-2 .5-2 2h2v4h-4V8z" />
    ),
    qa: (
      <g>
    ...
    article: (
      <g>
        ...
        <path d="M4 6h10M4 9h10M4 12h6" stroke="currentColor" strokeWidth="1.5" />
      </g>
    ),
    close: (
...
/* ── Status badge / depth marker / progress pills ── */
...
function DepthMarker({ depth }: { depth: string }) {
  const meta = DEPTH_META[depth as keyof typeof DEPTH_META];
  return (
...
function ProgressPills({ status }: { status: StoryStatus }) {
  const idx = STATUS_ORDER.indexOf(status);
  return (
    <span className="progress-pills">
      {STATUS_ORDER.map((s, i) => (
        <span key={s} className={`pill ${i <= idx ? (s === "committed" ? "filled-sun" : "filled") : ""}`} />
      ))}
  ...
  return (
    <nav className="v1-nav">
      <div className="v1-nav-inner">
        <a className="v1-nav-brand" href="/v1">
        ...
        <div className="v1-nav-links">
          ...
          <a href="https://community.alpha.school/?ref=UFB2FW8LX" target="_blank" rel="noopener noreferrer">
            Community
          </a>
        </div>
        <div className="v1-nav-right">
          <button className="v1-btn v1-btn-ghost v1-btn-sm">Log in</button>
          <button className="v1-btn v1-btn-primary v1-btn-sm">Join the hub</button>
        </div>
      </div>
    </nav>
  );
    ...
    <div className="community-hero">
      <div className="inner">
        <div className="hero-text-box">
          <h1>
            The families <em>behind</em> the families.
          </h1>
          <p className="lede">
            Before you tour, before you sign, before you move &mdash; hear from
            the parents who already did. Candid stories from Toronto families
            exploring, researching, committing to, and enrolled at Alpha.
          </p>
        </div>
...
function Featured({ story, onOpen }: { story: Story; onOpen: (s: Story) => void }) {
  return (
    <div className="featured">
      <div className="featured-inner">
        <div className="featured-img" style={{ backgroundImage: `url('${story.studentImg}')` }}>
          <span className="depth-tag">
            <Icon name="article" size={12} /> Long read &middot; 6 min
          </span>
        </div>
        <div className="featured-content">
          ...
          <div className="byline">
            ...
            <div>
              <b>{story.parent}</b>
              <div className="meta">
                {story.role} &middot; {story.neighbourhood}
              </div>
            </div>
          </div>
          <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
            <button className="v1-btn v1-btn-primary" onClick={() => onOpen(story)}>
              Read the full story
            </button>
...
function ShortCard({ story, onOpen, statusViz }: { story: Story; onOpen: (s: Story) => void; statusViz: string }) {
  return (
    <button className={`story-card depth-short st-${story.status}`} onClick={() => onOpen(story)}>
      ...
      <div className="card-body">
        ...
        {story.bigQuestion && <div className="card-question">{story.bigQuestion}</div>}
...
function QACard({ story, onOpen, statusViz }: { story: Story; onOpen: (s: Story) => void; statusViz: string }) {
  return (
...
function LongCard({ story, onOpen, statusViz }: { story: Story; onOpen: (s: Story) => void; statusViz: string }) {
  return (
    <button className={`story-card depth-long st-${story.status}`} onClick={() => onOpen(story)}>
      ...
      <div className="card-body">
        ...
        {story.bigQuestion && <div className="card-question" style={{ marginBottom: 14 }}>{story.bigQuestion}</div>}
...
function StoryCard({ story, onOpen, statusViz }: { story: Story; onOpen: (s: Story) => void; statusViz: string }) {
...
function Feed({ onOpen, layout, statusViz }: { onOpen: (s: Story) => void; layout: string; statusViz: string }) {
  ...
  const grades = useMemo(() => {
    ...
    STORIES.forEach((s) =>
      s.grade.split(",").map((g) => g.trim()).forEach((g) => all.add(g))
    );
    ...
    <div className="feed-section">
      <div className="feed-head">
        <div>
          ...
          <h2>
            All <em>{filtered.length}</em> stories, filtered to yours.
          </h2>
        </div>
      ...
      {filtered.length === 0 && (
        <div style={{ textAlign: "center", padding: "80px 20px", color: "var(--ink-4)" }}>
          ...
          <button className="v1-btn v1-btn-ghost" onClick={() => { setStatusFilter("all"); setGradeFilter("all"); }}>
            Clear filters
          </button>
        </div>
      )}
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="eyebrow">
            {story.family} &middot; {story.neighbourhood}
          </div>
          <h2>
            {story.title ? <>{story.title}</> : <>&ldquo;<em>{story.quote}</em>&rdquo;</>}
          </h2>
          {story.subtitle && <p className="lead">{story.subtitle}</p>}
            ...
            <div style={{ margin: "24px 0" }}>
              {story.qa.map((row, i) => (
                <div key={i} style={{ marginBottom: 28 }}>
                  ...
                  <h3 className="section-head" style={{ margin: "0 0 10px", fontSize: 22 }}>
                    {row.q}
                  </h3>
                  <p>{row.a}</p>
                </div>
          ...
          {story.depth === "short" && <p className="lead" style={{ marginTop: 16 }}>{story.excerpt}</p>}
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 32, flexWrap: "wrap" }}>
            <button className="v1-btn v1-btn-primary">Message {story.parent.split(" ")[0]}</button>
            <button className="v1-btn v1-btn-secondary" onClick={onClose}>Back to stories</button>
          </div>
        </div>
      </div>
    </div>
...
function FAQ({ onOpenStory }: { onOpenStory: (s: Story) => void }) {
      ...
      <div className="faq-inner">
        <div className="faq-head">
          <div>
            ...
            <h2>
              Every family <em>walks in</em> with the same six questions.
            </h2>
          </div>
          <p>
            Drawn directly from what hub families asked at their first Parent Lab
            &mdash; and the candid answers that came back from parents further
            along the journey.
          </p>
        </div>
        <div className="faq-grid">
          {FAQ_ITEMS.map((item, i) => {
            const askers = item.askedBy
                    ...
                    <div className="askers">
                      ...
                      <span className="count-text">
                        ...
                        <b style={{ color: "var(--ink-2)" }}>
                          {askers
                            .map((a) => a.family.replace("The ", "").replace(" Family", ""))
                            .join(", ")}
                        </b>
  ...
  return (
    <div className="share-cta">
      <div className="inner">
        ...
        <h2>
          You&rsquo;ve read ours. Tell us <em>yours</em>.
        </h2>
        <p>
          Whether you&rsquo;re one Instagram ad in or three years enrolled &mdash;
          your story helps the next family figure out whether Alpha is their
          shape. Fifteen minutes. We&rsquo;ll do the writing.
        </p>
      ...
      <div className="v1-footer-inner">
        <div>
          ...
          <img
            ...
            style={{ height: 50, marginBottom: 20, borderRadius: 4, background: "#fff", padding: 4 }}
            alt="Alpha Toronto"
          />
          <p style={{ color: "var(--alpha-sky)", fontSize: 14, maxWidth: "38ch", lineHeight: 1.5, margin: 0 }}>
            An unofficial community hub for Alpha School families in the GTA. Not
            operated by Alpha School.
          </p>
        </div>
        <div>
          <h5>Hub</h5>
...
export default function StoriesApp() {
  ...
  useEffect(() => {
    const id = localStorage.getItem("ath_story");
    if (id) {
  ...
  useEffect(() => {
    if (openStory) localStorage.setItem("ath_story", openStory.id);
    else localStorage.removeItem("ath_story");
  }, [openStory]);

  const featured = STORIES.find((s) => s.id === "lee")!;

  return (
    <>
      <Nav />
      <Hero />
      <Featured story={featured} onOpen={setOpenStory} />
      <Feed
        onOpen={setOpenStory}
        layout={tweaks.feedLayout}
        statusViz={tweaks.statusViz}
      />
      <FAQ onOpenStory={setOpenStory} />
      <ShareCTA />
      <Footer />
