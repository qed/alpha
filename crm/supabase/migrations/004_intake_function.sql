-- SECURITY DEFINER function for public intake form submissions
-- Runs with elevated privileges, bypasses RLS
-- INSERT-only grants: cannot read, update, or delete existing records
...
BEGIN
  ...
  IF v_existing_id IS NOT NULL THEN
    -- Re-submission: do NOT overwrite contact fields
    -- Return the existing ID so the app layer can log re-submission in activity feed
...
-- Grant execute to anon role (public intake form uses anon key)
GRANT EXECUTE ON FUNCTION public.submit_intake TO anon;

-- Revoke direct table access from anon — all intake goes through this function
